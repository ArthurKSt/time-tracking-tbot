...
def bd_getUser_perm_list(user_id, onlyAllow=False, onlyDeny=False):
    """return user's perms\n
    if onlyAllow return [permas]
    else {perms:param}"""
    ...
    if bd_dPerms:
        ...
        for indPerm, val in enumerate(bd_dPerms):
            ...
            if onlyDeny:
                if permParam == "allow": permParam = False
                elif permParam == "deny": permParam = True
                else: continue
